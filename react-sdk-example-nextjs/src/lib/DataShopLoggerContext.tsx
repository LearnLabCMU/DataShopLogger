...
export function DataShopLoggerProvider({ 
  ...
  const initialize = useCallback((initConfig: LogConfiguration) => {
    ...
    const nextjsSessionId = sessionToUse || `nextjs_session_${crypto.randomUUID ? crypto.randomUUID() : Date.now() + '_' + Math.random().toString(36).substring(2, 11)}`;
    ...
    if (!userGuid && typeof window !== 'undefined') {
      ...
      if (storedGuid) {
        ...
        userGuid = `nextjs-user-${crypto.randomUUID ? crypto.randomUUID() : Date.now() + '_' + Math.random().toString(36).substring(2, 11)}`;
    ...
    if (sessionToUse) {
      logger.resume(nextjsSessionId);
      finalSessionId = nextjsSessionId;
    } else {
